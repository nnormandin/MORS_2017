...
LEs <- TrainLEs(x = Xtrain, bincols = yb$cols, trControl = tc,
                method = 'gbm', n.folds = 5, tuneLength = 1,
                verbose = FALSE)
...
```{r, echo=TRUE, message=FALSE, warning=FALSE, cache = TRUE}
iso_mods <- bootstrap_iso(pm, yb$cols, NBOOT = 300, SAMP = 0.2)
pm2 <- predict_bootiso(iso_mods, pm)

LE_fits2 <- FitMatrix(as.matrix(pm2), yb$y.vals)
MLmetrics::RMSE(log(1 + LE_fits$mean), log(1 + ytrain))
MLmetrics::RMSE(log(1 + LE_fits2$mean), log(1 + ytrain))
MLmetrics::RMSE(log(1 + mod0_preds), log(1 + ytrain))
```
