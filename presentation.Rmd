...
trainset <- createDataPartition(y, p = 0.7)[[1]]
...
LEs <- TrainLEs(x = Xtrain, bincols = yb$cols, trControl = tc,
                method = 'gbm', n.folds = 8, tuneLength = 6,
                verbose = FALSE)
...
tmp <- FitInstance(LE_info$preds.matrix[1,], yb$y.vals, plot = TRUE)
```
...
\footnotesize
```{r, echo=FALSE, fig.height=5, fig.width=10, message=FALSE, warning=FALSE}
tmp <- FitInstance(LE_info$preds.matrix[2,], yb$y.vals, plot = TRUE)
```
...
\footnotesize
```{r, echo=FALSE, fig.height=5, fig.width=10, message=FALSE, warning=FALSE}
tmp <- FitInstance(LE_info$preds.matrix[3,], yb$y.vals, plot = TRUE)
```
